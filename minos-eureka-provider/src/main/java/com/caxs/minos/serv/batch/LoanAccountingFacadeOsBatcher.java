...
import com.caxs.minos.serv.db.ILoanAccountingFacadeOsBatcherService;
...
/***
 * @ClassName(类名) : LoanAccountingFacadeOsBatch
...
@Component
@Service
public class LoanAccountingFacadeOsBatcher implements ILoanAccountingFacadeOsBatcherService {
    //线程总数
    ...
    public LoanAccountingFacadeOsBatcher() {
        super();
    }
        ...
        if(batchJobContextTrans.getMutiThread()!=null &&
            ...
            while( itor2.hasNext() ){
                ...
                for (int i = 0; i < Integer.valueOf(batchJobContextTrans.getThreadCnt()); i++) {
                    ...
                    Thread thread = new Thread(new LoanAccountingFacadeOsJobBatcher(iNormIntAccrualSerivce,batchJobContext,sbch));
                    thread.start();
            ...
            while(itor2.hasNext()){
                SBch sbch = itor2.next();
                new LoanAccountingFacadeOsJobBatcher(iNormIntAccrualSerivce,batchJobContextTrans, sbch).run();
            }
        }
