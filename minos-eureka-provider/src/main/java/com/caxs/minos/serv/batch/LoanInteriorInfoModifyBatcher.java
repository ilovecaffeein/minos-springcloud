...
/***
 * @ClassName(类名) : LoanInteriorInfoModifyBatcher
...
@Component
@Service
public class LoanInteriorInfoModifyBatcher implements ILoanInteriorInfoModifyBatcherService {
    //线程总数
    ...
    public LoanInteriorInfoModifyBatcher() {
        super();
    }
    ...
    public void runBatch(BatchJobContextTrans batchJobContextTrans) {
        ...
        if ( batchJobContextTrans.getMutiThread() != null
            ...
            for (DeAtpyJobs job : jobList) {
                ...
                Thread thread = new Thread(new LoanInteriorInfoModifyJobBatcher(loanInteriorInfoModifyService,batchJobContext,job));
                thread.start();
            ...
            for (DeAtpyJobs job : jobList) {
                new LoanInteriorInfoModifyJobBatcher(loanInteriorInfoModifyService,batchJobContextTrans, job).run();
            }
        }
