...
import com.caxs.minos.serv.db.ILoanAmountAccBatcherService;
...
/***
 * @ClassName(类名) : LoanAmountAccBatch
 * @Description(描述) : 金额摊销
 * @author(作者) ： coffee
...
@Component
@Service
public class LoanAmountAccBatcher implements ILoanAmountAccBatcherService {
    //线程总数
        ...
        if(batchJobContextTrans.getMutiThread()!=null &&
            ...
            while( itor2.hasNext() ){
                ...
                for (int i = 0; i < Integer.valueOf(batchJobContextTrans.getThreadCnt()); i++) {
                    ...
                    Thread thread = new Thread(new LoanAmountAccJobBatcher(iNormIntAccrualSerivce,batchJobContext,sbch));
                    thread.start();
            ...
            while(itor2.hasNext()){
                SBch sbch = itor2.next();
                new LoanAmountAccJobBatcher(iNormIntAccrualSerivce,batchJobContextTrans, sbch).run();
            }
        }
