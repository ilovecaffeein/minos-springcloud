...
/***
 * @ClassName(类名) : LoanNormIntAccrualBatch
...
@Component
@Service
public class LoanNormIntAccrualBatcher implements ILoanNormIntAccrualBatcherService {
    //线程总数
    ...
    public LoanNormIntAccrualBatcher() {
        super();
    }
        ...
        if(batchJobContextTrans.getMutiThread()!=null &&
            ...
            while( itor2.hasNext() ){
                ...
                for (int i = 0; i < Integer.valueOf(batchJobContextTrans.getThreadCnt()); i++) {
                    ...
                    Thread thread = new Thread(new LoanNormIntAccrualJobBatcher(iNormIntAccrualSerivce,batchJobContext,sbch));
                    thread.start();
            ...
            while(itor2.hasNext()){
                SBch sbch = itor2.next();
                new LoanNormIntAccrualJobBatcher(iNormIntAccrualSerivce,batchJobContextTrans, sbch).run();
            }
        }
