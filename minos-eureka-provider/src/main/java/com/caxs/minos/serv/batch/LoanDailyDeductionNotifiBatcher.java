...
@Component
@Service
public class LoanDailyDeductionNotifiBatcher implements ILoanDailyDeductionNotifiBatcherService {
    ...
    public LoanDailyDeductionNotifiBatcher() {
        super();
    }
    ...
    public void runBatch(BatchJobContextTrans batchJobContext) {
        ...
        if (batchJobContextTrans.getMutiThread() != null
            ...
            for (DeAtpyJobs job : jobList) {
                ...
                Thread thread = new Thread(new LoanDailyDeductionNotifiJobBatcher(iDailyDeductionNotifiService,batchJobContextTrans,job));
                thread.start();
            ...
            for (DeAtpyJobs job : jobList) {
                new LoanDailyDeductionNotifiJobBatcher(iDailyDeductionNotifiService,batchJobContextTrans, job).run();
            }
        }
