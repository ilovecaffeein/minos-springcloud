...
/***
 * @ClassName(类名) : LoanSuspensionBatcher
...
@Component
@Service
public class LoanSuspensionBatcher implements ILoanSuspensionBatcherService {
    //线程总数
    ...
    public void runBatch(BatchJobContextTrans batchJobContextTrans) {
        ...
        if ( batchJobContextTrans.getMutiThread() != null
            ...
            for (DeAtpyJobs job : jobList) {
                ...
                Thread thread = new Thread(new LoanSuspensionJobBatcher(iSuspensionService,batchJobContext,job));
                thread.start();
            ...
            for (DeAtpyJobs job : jobList) {
                new LoanSuspensionJobBatcher(iSuspensionService,batchJobContextTrans, job).run();
            }
        }
